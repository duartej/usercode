...
process.load("MuonAnalysis.TagAndProbe.skim.skimUpsilon_cff")
...
from MuonAnalysis.TagAndProbe.skim.skimUpsilon_cff import *
...
#process.Skim_upsilonMu  = cms.Path(process.collisionFilter + process.upsilonMuFilter )
#process.Skim_upsilonTk  = cms.Path(process.collisionFilter + process.upsilonTkFilter )
#process.Skim_upsilonSta = cms.Path(process.collisionFilter + process.upsilonStaFilter)
process.Skim_upsilonMu  = cms.Path(process.upsilonMuFilter )
process.Skim_upsilonTk  = cms.Path(process.upsilonTkFilter )
process.Skim_upsilonSta = cms.Path(process.upsilonStaFilter)
...
process.oneTag1MuFilter  = process.upsilonMuFilter.clone(src = 'tags1Mu')
process.oneTag2MuFilter  = process.upsilonMuFilter.clone(src = 'tags2Mu')
...
##                   |_|              
##   
process.end = cms.EndPath(process.upsilonSkimOut)
...
#test = "OniaTrigger"
##del process.upsilonSkimOut.SelectEvents ## keep all events
#
#if len(args) > 0:
#    test = args[0]
...
#    process.source.fileNames = [ 'root://pcmssd12.cern.ch//data/mc/7TeV/Summer09/UpsilonMuMu_AODSIM_72B0D83F-908B-DE11-9A88-001D09646131.root' ]
#    process.upsilonSkimOut.fileName = 'UpsilonMuMu_skimUpsilon_Summmer09.root'
...
#    process.upsilonSkimOut.fileName = 'ppMuX_skimUpsilon_Summmer09.root'
#elif test == "RelVal354":
#    process.upsilonSkimOut.fileName = 'UpsilonMuMu_skimUpsilon_RelVal354.root'
#elif test == "OniaTrigger":
#    process.source.fileNames = [ 'root://pcmssd12.cern.ch//data/gpetrucc/PYTHIA6_7TeV_UpsilonWithFSR_MC_3XY_V16_1E31_HLT_12.root' ]
#    process.source.duplicateCheckMode = cms.untracked.string("noDuplicateCheck")
#    process.upsilonSkimOut.fileName = 'UpsilonMuMu_skimUpsilon_OniaTrigger.root'
#else:
